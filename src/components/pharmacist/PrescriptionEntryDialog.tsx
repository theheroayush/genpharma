...
const frequencyOptions = [
    { value: "once_daily", label: "Once Daily", defaultSlots: ["morning"] as TimeSlot[] },
    { value: "twice_daily", label: "Twice Daily", defaultSlots: ["morning", "evening"] as TimeSlot[] },
    { value: "thrice_daily", label: "Three Times Daily", defaultSlots: ["morning", "afternoon", "evening"] as TimeSlot[] },
    { value: "every_8h", label: "Every 8 Hours", defaultSlots: ["morning", "afternoon", "night"] as TimeSlot[] },
    { value: "at_bedtime", label: "At Bedtime", defaultSlots: ["night"] as TimeSlot[] },
];
...
export function PrescriptionEntryDialog({ open, onClose, onSave }: Props) {
    ...
    const [suggestions, setSuggestions] = useState<ReturnType<typeof searchDrugs>>([]);
    ...
    const handleAddMedication = () => {
        if (!selectedDrug || !dosage || !frequency) {
            toast({ title: "Missing fields", description: "Please select a drug, dosage, and frequency.", variant: "destructive" });
            return;
        }
        ...
        // Reset
        setDrugSearch(""); setSelectedDrug(""); setSelectedGeneric(""); setDosage("");
        setFrequency(""); setDuration("30"); setInstructions(""); setAvailableDosages([]);
    };
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = () => {
            ...
            // Simulated OCR
            setOcrText(`[OCR Scan Result]\nPatient: ${patientName || "Unknown"}\nMedications detected from image.\nPlease verify and add medications manually using the drug search above.`);
            toast({ title: "Prescription scanned", description: "OCR completed. Please verify detected information." });
        };
    ...
    const handleSave = () => {
        ...
        onSave(prescription);
        toast({ title: "Prescription created", description: `${id} for ${patientName} with ${medications.length} medication(s).` });
        // Reset all
        setPatientName(""); setPatientAge(""); setCondition(""); setMedications([]);
        setImageUrl(undefined); setOcrText(undefined);
        onClose();
    };
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <Input value={patientName} onChange={(e) => setPatientName(e.target.value)} placeholder="e.g. Rajesh Kumar" />
                        </div>
                        <div>
                            <Label>Age</Label>
                            <Input type="number" value={patientAge} onChange={(e) => setPatientAge(e.target.value)} placeholder="e.g. 54" />
                        </div>
                        <div>
                            <Label>Condition</Label>
                            <Input value={condition} onChange={(e) => setCondition(e.target.value)} placeholder="e.g. Diabetes Type 2" />
                        </div>
                    </div>
                        ...
                        >
                            {imageUrl ? (
                                ...
                                <div className="flex flex-col items-center gap-2 text-muted-foreground">
                                    ...
                                    <span className="text-sm">Click to upload prescription photo</span>
                                </div>
                            )}
                    ...
                    <div className="border rounded-lg p-4 space-y-3">
                        ...
                        <div className="relative">
                            ...
                            <Input
                                value={drugSearch}
                                onChange={(e) => { setDrugSearch(e.target.value); setSelectedDrug(""); }}
                        ...
                        <div>
                            ...
                            <Textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} placeholder="e.g. Take after meals" rows={2} />
                        </div>

                        <Button variant="outline" size="sm" className="gap-1" onClick={handleAddMedication}>
                    ...
                    {medications.length > 0 && (
                        <div className="space-y-2">
                            <h4 className="font-semibold text-sm">Medications ({medications.length})</h4>
                            {medications.map((med, i) => (
                                <div key={i} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                                    <div>
                                        <p className="font-medium text-sm">{med.drugName} — {med.dosage}</p>
                                        <p className="text-xs text-muted-foreground">
                                            {frequencyOptions.find((f) => f.value === med.frequency)?.label} · {med.duration} days
                                            {med.specialInstructions && ` · ${med.specialInstructions}`}
                                        </p>
                                        <div className="flex gap-1 mt-1">
                                            {med.timeSlots.map((s) => (
                                                <Badge key={s} variant="outline" className="text-[10px]">{timeSlotLabels[s]}</Badge>
                                            ))}
                                        </div>
                                    </div>
                                    <Button variant="ghost" size="icon" aria-label="Remove medication" onClick={() => removeMedication(i)}>
